...
const ManagerAppointments = () => {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'Pending':
        return <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#F59E0B]/10 text-[#F59E0B] border border-[#F59E0B]/20 flex items-center gap-1"><Clock size={12} /> Pending</span>;
      case 'Rejected':
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 bg-slate-100 dark:bg-slate-800/50 p-1 rounded-lg">
          <button 
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'dashboard' ? 'bg-white dark:bg-slate-700 text-[#F59E0B] shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            Dashboard
          </button>
          <button 
            onClick={() => setActiveTab('request')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'request' ? 'bg-gradient-to-r from-[#F59E0B] to-yellow-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            <Plus size={16} />
            New Request
          </button>
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] rounded-2xl p-6 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl -mr-16 -mt-16 transition-transform group-hover:scale-150 duration-700"></div>
            ...
            <div className="bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] rounded-2xl p-6 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#F59E0B]/5 rounded-full blur-3xl -mr-16 -mt-16 transition-transform group-hover:scale-150 duration-700"></div>
              <div className="flex justify-between items-start mb-4">
          ...
          <div className="bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-[#334155] flex justify-between items-center">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search requests..." className="pl-9 pr-4 py-2 border border-slate-200 dark:border-[#334155] rounded-lg bg-slate-50 dark:bg-[#0F172A] text-sm focus:ring-2 focus:ring-[#F59E0B] focus:border-transparent outline-none w-64 dark:text-white" />
              </div>
            </div>
                    ...
                    <tr>
                      <td colSpan="6" className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <button onClick={() => setActiveTab('request')} className="mt-4 px-4 py-2 bg-gradient-to-r from-[#F59E0B] to-yellow-600 text-white rounded-lg hover:shadow-lg transition-all">Create Request</button>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    appointments.map((appt) => (
                      <tr key={appt.id} className="hover:bg-slate-50 dark:hover:bg-[#0F172A]/50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <button className="text-[#F59E0B] hover:underline text-sm font-medium">View Details</button>
                        </td>
                      </tr>
                    ))
                  )}
      ...
      {activeTab === 'request' && (
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSubmit} className="bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-[#334155] bg-slate-50/50 dark:bg-[#0F172A]">
            ...
            <div className="p-6 md:p-8 space-y-8">
              {/* Meeting Details */}
              <div>
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-200 dark:border-[#334155] pb-2">Meeting Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Subject / Title *</label>
                    <input type="text" name="subject" value={formData.subject} onChange={handleInputChange} required className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow" placeholder="e.g. Quarterly Retail Branch Review" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Primary Purpose *</label>
                    <input type="text" name="purpose" value={formData.purpose} onChange={handleInputChange} required className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow" placeholder="Brief 1-sentence summary of the meeting goal" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Meeting Type *</label>
                    <select name="meetingType" value={formData.meetingType} onChange={handleInputChange} required className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow">
                  ...
                  <div>
                    ...
                    <select name="priority" value={formData.priority} onChange={handleInputChange} required className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow">
                      <option>Low</option>
              ...
              <div>
                <div className="flex justify-between items-end mb-4 border-b border-slate-200 dark:border-[#334155] pb-2">
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Select Available Slot *</h3>
                  <span className="text-xs text-[#F59E0B] bg-[#F59E0B]/10 px-2 py-1 rounded">Smart Date Selection</span>
                </div>
                
                {loading ? (
                  <div className="p-8 text-center text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-[#334155] border-dashed rounded-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                    {availableSlots.map(slot => (
                      <div 
                        ...
                        className={`cursor-pointer border rounded-xl p-4 transition-all ${formData.preferredSlotId === slot.id ? 'border-[#F59E0B] bg-[#F59E0B]/5 ring-1 ring-[#F59E0B]' : 'border-slate-200 dark:border-[#334155] hover:border-[#F59E0B]/50 bg-white dark:bg-[#0F172A]'}`}
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className="font-bold text-slate-900 dark:text-white">{slot.date}</span>
                          {formData.preferredSlotId === slot.id && <CheckCircle size={18} className="text-[#F59E0B]" />}
                        </div>
              ...
              {/* Extended Details */}
              <div>
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-200 dark:border-[#334155] pb-2">Extended Details</h3>
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Detailed Agenda *</label>
                    <textarea name="agenda" value={formData.agenda} onChange={handleInputChange} required rows="4" className="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow resize-none" placeholder="1. Project Status Update (10m)&#10;2. Q3 Financial Projections (15m)&#10;3. Key Decisions Needed (5m)"></textarea>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" name="participants" value={formData.participants} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow" placeholder="e.g. Jane Doe, John Smith" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Additional Remarks</label>
                      <input type="text" name="remarks" value={formData.remarks} onChange={handleInputChange} className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white focus:ring-2 focus:ring-[#F59E0B] outline-none transition-shadow" placeholder="Any special requirements..." />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 border-t border-slate-200 dark:border-[#334155] bg-slate-50 dark:bg-[#0F172A] flex justify-end gap-3">
              <button 
                type="button"
                onClick={() => setActiveTab('dashboard')}
                className="px-6 py-2.5 rounded-xl border border-slate-300 dark:border-[#334155] text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#1E293B] font-medium transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                className={`px-8 py-2.5 rounded-xl text-white font-medium shadow-sm transition-all flex items-center ${submitting || !formData.preferredSlotId ? 'bg-slate-400 dark:bg-slate-600 cursor-not-allowed' : 'bg-gradient-to-r from-[#F59E0B] to-yellow-600 hover:shadow-lg'}`}
              >
